...
double latitudeFromLatitudeISO(double lat_iso, double e,double eps)
{
  ...
  double phi_i = 2*atan(pow((1+e*sin(phi_0))/(1-e*sin(phi_0)),e/2.0)*exp(lat_iso)) - PI/2;
  double delta = phi_i - phi_0;

  while(delta.abs() > eps)
  {
    phi_0 = phi_i;
    phi_i = 2*atan(pow((1+e*sin(phi_0))/(1-e*sin(phi_0)),e/2.0)*exp(lat_iso)) - PI/2;
...
pt.Point lambertToGeographic(pt.Point org, lambertZone.Zone zone, double lon_merid, double e, double eps) {
...
pt.Point geographicToCartesian(double lon, double lat, double he, double a, double e)
{
...
pt.Point cartesianToGeographic(pt.Point org, double meridien, double a, double e , double eps)
{
